  ...
  return <section id="activities" className="min-h-screen py-12 px-6">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex flex-wrap justify-center gap-4 mb-8">
          {tabs.map(tab => {
          const Icon = tab.icon;
          return <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`flex items-center gap-2 px-6 py-3 rounded-lg font-medium transition-all duration-200 ${
            activeTab === tab.id 
              ? 'bg-gradient-to-r from-blue-600 to-blue-800 text-white shadow-lg' 
              : 'bg-white/80 text-gray-600 hover:bg-blue-50/30'
          }`}>
                <Icon size={18} />
                {tab.label}
              </button>;
        ...
        <div className="bg-white/80 backdrop-blur-sm border border-slate-200 rounded-2xl p-8">
          <div className="space-y-6">
            {activities[activeTab as keyof typeof activities].map((activity, index) => <div key={index} className="timeline-item animate-fade-in hover:bg-blue-50/30 transition-colors rounded-lg p-4" style={{
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-2">
                      ...
                      <span className="bg-gradient-to-r from-blue-600 to-blue-800 px-3 py-1 rounded-full text-sm font-medium text-white">
                        {activity.year}
                      </span>
                    </div>
